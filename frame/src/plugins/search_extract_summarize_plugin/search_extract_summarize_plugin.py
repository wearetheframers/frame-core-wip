...
class SearchExtractSummarizePlugin(PluginBase):
    ...
    def load_dependencies(self) -> bool:
        ...
        if missing_modules:
            self.logger.warning(f"Missing dependencies for SearchExtractSummarizePlugin: {', '.join(missing_modules)}")
            self.logger.info("To use this plugin, install the required dependencies with:")
    ...
    def read_env_variables(self) -> None:
        ...
        if not all([self.search_api_key, self.search_project_id, self.llm_api_key]):
            self.logger.warning("Missing required environment variables for SearchExtractSummarizePlugin")
            return
    ...
    async def on_load(self):
        if not self.dependencies_loaded:
            self.logger.warning("SearchExtractSummarizePlugin is disabled due to missing dependencies")
            return
    ...
    async def search_extract_summarize(self, query: str, date_restrict: Optional[int] = None, target_site: Optional[str] = None, model_name: str = "gpt-4o-mini") -> str:
        if not self.dependencies_loaded:
        ...
        references = "\n".join([f"[{i+1}] {result['metadata']['url']}" for i, result in enumerate(results)])
        return f"# Answer\n\n{answer}\n\n# References\n\n{references}"

    def search_web(self, query: str, date_restrict: Optional[int], target_site: Optional[str]) -> List[str]:
        ...
        response = requests.get(search_url, headers={"Authorization": f"Bearer {self.search_api_key}"})
    ...
    def chunk_results(self, scrape_results: Dict[str, str], size: int, overlap: int) -> Dict[str, List[str]]:
        chunked_results = {}
    ...
    def vector_search(self, query: str) -> List[Dict[str, Any]]:
        ...
        return [{"chunk": result.content, "metadata": result.metadata} for result in results]
    ...
    async def run_inference(self, query: str, model_name: str, matched_chunks: List[Dict[str, Any]]) -> str:
        prompt_template = """
        ...
        prompt = self._render_template(prompt_template, {"matched_chunks": matched_chunks, "query": query})
