...
- **Framer**: Represents an individual AI agent equipped with advanced capabilities for task management, decision-making, and interaction with language models. The Soul, Agency, and ExecutionContext components are integral to a Framer's flexibility and customization, making it adaptable for a wide range of applications. These components are the primary interfaces through which users will interact with the API. The `halt()` method can be used to stop the Framer from acting, providing control over its activity.
  - Initialization: During creation, Framers automatically load and initialize plugins based on their configuration and permissions.
  - Perception Processing: Framers can sense and process various types of incoming data (perceptions). These perceptions are not limited to human senses and can include any type of information or stimulus, such as text, images, sounds, magnetic fields, vibrations, internal states like hunger, or any other data that can be analyzed by the language model. The "prompt" action in a Framer is essentially processing a perception of hearing for text input and responding to it.
  ...
  - Plugins: Extend the Framer's behavior with new actions. Plugins are loaded and initialized during Framer creation based on the Framer's configuration and permissions. When a plugin action is well-described, the Framer can make reasonable moves to take that action based on context, internal thinking, and the action's priority compared to other possible actions.
  - Permissions: Determine which plugins and services a Framer has access to, controlling its capabilities.
  - Roles: Framer roles are represented by the `Role` class, which includes attributes such as id, name, description, permissions, priority, and status. Multiple roles can be active simultaneously, allowing the Framer to fulfill various functions concurrently. Roles can have different statuses (ACTIVE, INACTIVE, SUSPENDED) to reflect their current state. The priority of roles influences decision-making, with higher priority roles having more impact.
  - Goals: Goals are represented by the `Goal` class, which includes attributes such as name, description, priority, and status. Multiple goals can be active at the same time, enabling the Framer to pursue various objectives concurrently. Goals guide the Framer's decision-making process and can have different statuses (ACTIVE, COMPLETED, ABANDONED) to reflect their current state. The priority of goals affects the Framer's focus, with higher priority goals being given more attention in decision-making.
  - Tasks: Tasks are represented by the `Task` class and use the same Priority component as roles and goals. Multiple tasks can be active and in progress simultaneously, allowing the Framer to work on various actions concurrently. The priority of a task influences its execution order and importance within workflows.
  - Priority System: Roles, goals, and tasks use a shared Priority component (LOW, MEDIUM, HIGH, CRITICAL) to determine their importance in the Framer's decision-making process. This allows for dynamic and context-aware behavior, where the Framer can adjust its actions based on the current priorities of its active roles, goals, and tasks.
...
Set permissions when creating a Framer:
...
This gives the Framer access to the Memory service, EQ service, and the Search Extract Summarize plugin.

Frame features a plugin marketplace for premium and community plugins, fostering a rich ecosystem of extensions and customizations.

Note: Some plugins may require additional API keys or configurations. Refer to specific plugin documentation for requirements.
...
- **Actions**: Represents specific operations that a Framer can perform, such as research, respond, or create new agents.
