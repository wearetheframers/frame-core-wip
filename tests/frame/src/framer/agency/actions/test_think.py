...
async def test_think(mock_framer):
    ...
    mock_framer.brain.make_decision.assert_called_once_with({"action": "think", "parameters": {"thought": thought}})
    mock_framer.brain._execute_think_action.assert_called_once()
    assert result == {"analysis": "Test analysis"}

@pytest.mark.asyncio
async def test_think_default_thought(mock_framer):
    mock_framer.brain._execute_think_action.return_value = {"analysis": "Default analysis"}
    
    result = await think(mock_framer)

    mock_framer.brain.make_decision.assert_called_once_with({"action": "think", "parameters": {"thought": "Processing information..."}})
    mock_framer.brain._execute_think_action.assert_called_once()
