...
def run_example(example):
    ...
    subprocess.run(["cmd", "/c", "venv\\Scripts\\activate && python", os.path.join(os.path.dirname(__file__), example)])
...
def main():
    ...
    if examples[choice - 1] == "security_system_example.py":
        ...
        print("The AI will intelligently choose to activate security based on the time of night and this tolerance level.")
        print("For example, if the tolerance level is low, the AI may activate security earlier in the evening.")
    try:
        choice = int(choice)
